"""DEC (Discrete Exterior Calculus) shape features for the full-body crash surrogate.
...
node area, and an open-boundary flag. These are static input features —
non-trivial on the reference mesh, invariant/equivariant under rigid motion,
and mesh-density corrected by the barycentric mass matrix (the DEC Hodge
star_0).

Port of examples/structural_mechanics/crash/dec_features.py (kept in sync by
hand — each example directory is self-contained, mirroring vtkhdf_reader.py).
Adds `triangulate_flat` for the VTK-style flat (connectivity, offsets) cell
representation returned by this example's reader.

Self-test (flat plane / sphere / rigid-motion invariance):
    python3 -m geo_encoders.dec_features
"""
...
# Feature keys produced by dec_point_features, and their channel widths.
...
def triangulate_flat(conn: np.ndarray, offsets: np.ndarray) -> np.ndarray:
    """[M,3] triangles from flat VTK-style connectivity + offsets (vectorized).

    Same semantics as `triangulate_cells`: 3-node cells kept, 4-node cells
    split along the (0,2) diagonal, other sizes skipped.
    """
    ...
    s3 = starts[sizes == 3]
    tris3 = np.stack([conn[s3], conn[s3 + 1], conn[s3 + 2]], axis=1) if len(s3) \
        else np.zeros((0, 3), np.int64)

    s4 = starts[sizes == 4]
    if len(s4):
        a, b, c, d = conn[s4], conn[s4 + 1], conn[s4 + 2], conn[s4 + 3]
        quads = np.concatenate(
            [np.stack([a, b, c], axis=1), np.stack([a, c, d], axis=1)]
        )
    else:
        quads = np.zeros((0, 3), np.int64)

    return np.concatenate([tris3, quads]) if (len(tris3) or len(quads)) \
        else np.zeros((0, 3), np.int64)
...
def dec_point_features_from_tris(coords0: np.ndarray, tris: np.ndarray) -> dict:
    """Per-node DEC features given a pre-triangulated mesh."""
...
def dec_point_features(coords0: np.ndarray, cells) -> dict:
    ...
    """
    return dec_point_features_from_tris(coords0, triangulate_cells(cells))


def dec_point_features_flat(
    coords0: np.ndarray, conn: np.ndarray, offsets: np.ndarray
) -> dict:
    """Like `dec_point_features`, but for flat (connectivity, offsets) cells."""
    return dec_point_features_from_tris(coords0, triangulate_flat(conn, offsets))


# ----------------------------------------------------------------------------
# Self-tests (flat plane, sphere, rigid-motion invariance, flat == list cells)
# ----------------------------------------------------------------------------
...
def _to_flat(cells):
    conn = np.concatenate([np.asarray(c) for c in cells]).astype(np.int64)
    offsets = np.concatenate([[0], np.cumsum([len(c) for c in cells])]).astype(np.int64)
    return conn, offsets
...
def run_tests():
    ...
    # flat (connectivity, offsets) path must match the list-of-cells path
    conn, offsets = _to_flat(cs)
    ff = dec_point_features_flat(Xs, conn, offsets)
    flat_ok = all(np.array_equal(ff[k], fs[k]) for k in DEC_FEATURE_KEYS)
    print("[test] flat conn/offsets path matches list-of-cells path",
